...
import { ArrowUpRight, Heart, Plus, Search, Users, Lightbulb, ArrowRight } from 'lucide-react';
import type { IdeaItem, Student } from '@/lib/types';
import { useCircle } from './circle-context';
import { Avatar, Empty, Modal, Tag, relative } from './ui';
import { PageHeading } from './pages';
type Resonance = { user: Student; userId: string; createdAt: string };
export function IdeaCard({ idea, compact = false }: { idea: IdeaItem; compact?: boolean }) {
  const { state, mutate, busy, viewProfile } = useCircle();
      ...
      <article className={`idea-card ${compact ? 'compact' : ''}`}>
        ...
        <div className="idea-bottom">
          <button
            ...
            onClick={async () => {
              try {
                await mutate(`ideas/${idea.id}/resonate`, { enabled: !resonated });
              } catch {}
            }}
          >
...
function IdeaDetail({ idea: initial, onClose }: { idea: IdeaItem; onClose: () => void }) {
  ...
  const mine = idea.authorId === state.me.id;
  const [people, setPeople] = useState<Resonance[]>([]);
  ...
  useEffect(() => {
    if (!mine) return;
    const load = () =>
      api<Resonance[]>(`ideas/${idea.id}/resonances`)
        .then(setPeople)
      ...
      <div className="idea-detail">
        ...
        {mine ? (
          <section className="resonance-section">
            ...
            <div className="dialog-actions">
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    const group = await mutate<{ id: string }>(`ideas/${idea.id}/group`, {
                      memberIds: selected,
                    });
                    toast(
                      idea.conversation
          ...
          <button
            ...
            onClick={async () => {
              try {
                await mutate(`ideas/${idea.id}/resonate`, { enabled: !idea.resonances.length });
              } catch {}
            }}
          >
            <Heart size={16} />
