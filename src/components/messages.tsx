...
import type { ChatMessage, ConversationItem, Student } from '@/lib/types';
import { useCircle } from './circle-context';
import { Avatar, Empty, Modal, relative } from './ui';
...
export function MessagesPage() {
  ...
  const loadMessages = useCallback(async () => {
    if (!selected) return;
    try {
      const result = await api<ChatMessage[]>(`conversations/${selected}/messages`);
  ...
  async function send(event: React.FormEvent) {
    ...
    try {
      await mutate(`conversations/${selected}/messages`, payload);
      setText('');
      retryMessage.current = null;
      await loadMessages();
    } catch {
                    ...
                    onClick={async () => {
                      try {
                        const older = await api<ChatMessage[]>(
                          `conversations/${selected}/messages?before=${encodeURIComponent(messages[0].id)}`,
                        );
    ...
    <Modal title="Bring your people together." onClose={onClose}>
      <form
        onSubmit={async (e) => {
          ...
          setError('');
          try {
            const group = await mutate<{ id: string }>('conversations', {
              type: 'GROUP',
...
function ManageGroup({
  ...
  useEffect(() => {
    ...
    if (conversation.ideaId && owner)
      api<{ user: Student }[]>(`ideas/${conversation.ideaId}/resonances`)
  ...
  async function action(action: string, userId?: string, value?: string) {
    try {
      await mutate(`conversations/${conversation.id}`, { action, userId, value }, 'PATCH');
